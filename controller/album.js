...
export const edit = async (req, res) => {
  ...
  try {
    let result = await album.findById(req.params.id)
    if (result === null) {
      res.status(404).send({ success: false, message: '找不到資料' })
    } else if (result.user !== req.session.user._id) {
      res.status(403).send({ success: false, message: '沒有權限' })
    } else {
      result = await album.findByIdAndUpdate(req.params.id, req.body, { new: true })
      res.status(200).send({ success: true, message: '', result })
    }
  } catch (error) {
    console.log(error)
...
export const deletee = async (req, res) => {
  ...
  try {
    ...
    if (result === null) {
      ...
      result = await album.findByIdAndDelete(req.params.id)
      res.status(200).send({ success: true, message: '', result })
    }
  } catch (error) {
    console.log(error)
...
export const user = async (req, res) => {
  ...
  try {
    const result = await album.find({ user: req.params.user })
    res.status(200).send({ success: true, message: '', result })
  } catch (error) {
    res.status(500).send({ success: false, message: '伺服器錯誤' })
  }
...
export const file = async (req, res) => {
  ...
  if (process.env.DEV === 'true') {
    ...
    if (exists) {
      ...
      res.stat(404).send({ success: false, message: '找不到圖片' })
    }
  } else {
    axios({
      method: 'GET',
